...
# 导入数据
train_x = scio.loadmat('ex8data1.mat')['X']
validation_x = scio.loadmat('ex8data1.mat')['Xval']
validation_y = scio.loadmat('ex8data1.mat')['yval']
...
# # 计算每个样本的概率
# train_p = compute_p1(train_x, mu, sigma)  # 方法1.直接按列相乘
...
y = np.arange(5, 25, 0.5)
xx, yy = np.meshgrid(x, y)
xxyy = np.column_stack((xx.reshape(-1, 1), yy.reshape(-1, 1)))
# zz = compute_p1(xxyy, mu, sigma).reshape(xx.shape)  # 方法1
zz = compute_p2(xxyy, mu, cov).reshape(xx.shape)  # 方法2
# plt.contourf(xx, yy, zz)  # 填充颜色
plt.contour(xx, yy, zz)  # 画等高线
plt.scatter(train_x[:, 0], train_x[:, 1], s=5)  # 样本分布
plt.show()

# 选择最优的阈值
validation_p = compute_p2(validation_x, mu, cov)  # 计算验证集样本的概率（方法2）
epsilons = np.arange(10**(-7), 10**(-3), 10**(-6))  # 阈值的取值范围
F1 = {'epsilon': [], 'F1': []}
...
plt.scatter(anomalies[:, 0], anomalies[:, 1], c='r', s=100, marker='o')  # 异常样本
plt.scatter(train_x[:, 0], train_x[:, 1], c='b', s=5)  # 所有样本
plt.show()
